...
class MainActivity: FlutterActivity() {
        ...
        contentResolver.query(childrenUri, arrayOf(DocumentsContract.Document.COLUMN_DOCUMENT_ID, DocumentsContract.Document.COLUMN_DISPLAY_NAME, DocumentsContract.Document.COLUMN_MIME_TYPE), null, null, null)?.use { cursor ->
            while (cursor.moveToNext()) {
                if (cursor.getString(1) == name) {
                    val id = cursor.getString(0); val mime = cursor.getString(2)
                    val itemUri = DocumentsContract.buildDocumentUriUsingTree(parent.uri, id)
                    val found = if (mime == DocumentsContract.Document.MIME_TYPE_DIR) DocumentFile.fromTreeUri(this, itemUri) else DocumentFile.fromSingleUri(this, itemUri)
    ...
    private fun uploadFileNative(url: String, token: String?, masterKey: String?, remotePath: String, uriStr: String, hash: String, deviceName: String, updatedAt: Long, dirtyIndices: List<Int>?, result: MethodChannel.Result) {
        acquireWakeLock()
        Thread {
            var pfd: ParcelFileDescriptor? = null; var channel: FileChannel? = null
            try {
                val utf8 = Charsets.UTF_8; val magic = "NEOSYNC".toByteArray(utf8); val keyBytes = if (masterKey != null) android.util.Base64.decode(masterKey, android.util.Base64.URL_SAFE).sliceArray(0 until 32) else null
                if (isShizukuPath(uriStr)) { pfd = getShizukuServiceSync().openFile(getCleanPath(uriStr), "r") ?: throw Exception("PFD failed"); channel = FileInputStream(pfd!!.fileDescriptor).channel } 
                else if (uriStr.startsWith("content://")) { pfd = contentResolver.openFileDescriptor(Uri.parse(uriStr), "r") ?: throw Exception("PFD failed"); channel = FileInputStream(pfd!!.fileDescriptor).channel } 
                else { channel = RandomAccessFile(File(uriStr), "r").channel }
                ...
                for (index in indicesToSync) {
                    val offset = index.toLong() * blockSize; val buffer = ByteBuffer.allocate(blockSize); channel!!.position(offset); val bytesRead = channel!!.read(buffer)
                    if (bytesRead == -1 && index != 0) continue; val blockData = if (bytesRead <= 0) ByteArray(0) else buffer.array().sliceArray(0 until bytesRead)
                    val finalData: ByteArray = if (keyBytes != null && blockData.isNotEmpty()) {
                        val iv = MessageDigest.getInstance("MD5").digest(blockData); val cipher = Cipher.getInstance("AES/CBC/PKCS7Padding").apply { init(Cipher.ENCRYPT_MODE, SecretKeySpec(keyBytes, "AES"), IvParameterSpec(iv)) }
                    ...
                    val connection = (java.net.URL(url).openConnection() as java.net.HttpURLConnection).apply { requestMethod = "POST"; doOutput = true; setFixedLengthStreamingMode(finalData.size.toLong()); setRequestProperty("Content-Type", "application/octet-stream"); setRequestProperty("x-vaultsync-path", remotePath); setRequestProperty("x-vaultsync-index", index.toString()); val overhead = if (keyBytes != null) (7 + 16 + 16) else 0; setRequestProperty("x-vaultsync-offset", (index.toLong() * (blockSize + overhead)).toString()); if (token != null) setRequestProperty("Authorization", "Bearer $token"); connectTimeout = 10000; readTimeout = 30000 }
                    BufferedOutputStream(connection.outputStream).use { it.write(finalData); it.flush() }; if (connection.responseCode != 200) throw Exception("Block $index: HTTP ${connection.responseCode}")
                }
                ...
                if (finalizeConn.responseCode != 200) throw Exception("Finalization failed"); runOnUiThread { result.success(true) }
            ...
            finally { try { channel?.close() } catch(_: Exception) {}; try { pfd?.close() } catch(_: Exception) {}; releaseWakeLock() }
        }.start()
    }
    ...
    private fun downloadFileNative(url: String, token: String?, masterKey: String?, remoteFilename: String, uriStr: String, localFilename: String, updatedAt: Long?, patchIndices: List<Int>?, result: MethodChannel.Result) {
        ...
        Thread {
            ...
            var channel: FileChannel? = null; var pfd: ParcelFileDescriptor? = null; var srcPfd: ParcelFileDescriptor? = null
            try {
                ...
                connection = (java.net.URL(downloadUrl).openConnection() as java.net.HttpURLConnection).apply { requestMethod = "POST"; doOutput = true; setRequestProperty("Content-Type", "application/json"); if (token != null) setRequestProperty("Authorization", "Bearer $token"); connectTimeout = 10000; readTimeout = 30000 }
                ...
                val blockSize = 1024 * 1024; val encryptedBlockSize = blockSize + 7 + 16 + 16
                ...
                if (isShizukuPath(uriStr)) {
                    ...
                    val service = getShizukuServiceSync(); pfd = service.openFile(tmpPath, "rw") ?: throw Exception("PFD failed"); channel = FileOutputStream(pfd!!.fileDescriptor).channel
                    ...
                    pfd = contentResolver.openFileDescriptor(safTmp!!.uri, "rw") ?: throw Exception("PFD failed"); channel = FileOutputStream(pfd!!.fileDescriptor).channel; targetPath = null
                    ...
                    if (patchIndices != null && f.exists()) f.copyTo(tmpFile, overwrite = true); channel = RandomAccessFile(tmpFile, "rw").channel
                }
                channel!!.use { output ->
                    connection!!.inputStream.use { input ->
                        ...
                        while (true) {
                            ...
                            while (buffer.size() >= currentBSize) {
                                ...
                                val decrypted: ByteArray = if (keyBytes != null && block.sliceArray(0..6).contentEquals(magic)) { val iv = block.sliceArray(7..22); val encrypted = block.sliceArray(23 until currentBSize); val cipher = Cipher.getInstance("AES/CBC/PKCS7Padding").apply { init(Cipher.DECRYPT_MODE, SecretKeySpec(keyBytes!!, "AES"), IvParameterSpec(iv)) }; cipher.doFinal(encrypted) } else { block }
                    ...
                    if (Build.VERSION.SDK_INT >= 30) try { channel!!.force(true) } catch(_: Exception) {}
                }
                if (isShizukuPath(uriStr)) { getShizukuServiceSync().renameFile("${targetPath}.vstmp", targetPath!!); if (updatedAt != null) setFileTimestamp("shizuku://$targetPath", updatedAt) } 
                else if (safTmp != null) { val finalName = localFilename.split("/").last(); synchronized(safLock) { if (!safTmp!!.renameTo(finalName)) { val existingFinal = safFindFileStrict(parentDir!!, finalName); existingFinal?.delete(); val newFinal = parentDir!!.createFile("application/octet-stream", finalName) ?: throw Exception("Fallback failed"); contentResolver.openInputStream(safTmp!!.uri)?.use { input -> contentResolver.openOutputStream(newFinal.uri)?.use { out -> input.copyTo(out) } }; safTmp!!.delete() } } } 
                else { val f = File(targetPath!!); val tmp = File("$targetPath.vstmp"); if (f.exists()) f.delete(); tmp.renameTo(f); if (updatedAt != null) setFileTimestamp(f.absolutePath, updatedAt) }
            ...
            finally { connection?.disconnect(); try { channel?.close() } catch(_: Exception) {}; try { pfd?.close() } catch(_: Exception) {}; try { srcPfd?.close() } catch(_: Exception) {}; releaseWakeLock() }
        }.start()
    }
    ...
    private fun scanRecursive(path: String, systemId: String, ignoredFoldersList: List<String>, result: MethodChannel.Result) {
        Thread {
            try {
                ...
                val syncEverything = sid == "switch" || sid == "ps2" || sid == "psx" || sid == "ps1" || sid == "duckstation" || sid == "aethersx2" || sid == "nethersx2" || sid == "pcsx2"

                if (path.startsWith("content://")) {
                    val uri = Uri.parse(path); val treeUri = if (DocumentsContract.isTreeUri(uri)) uri else null; val startDocId = try { DocumentsContract.getTreeDocumentId(uri) } catch (e: Exception) { null }
                    if (startDocId != null) {
                        fun walkSaf(currentDocId: String, currentRelPath: String, depth: Int) {
                            ...
                            contentResolver.query(childrenUri, arrayOf(DocumentsContract.Document.COLUMN_DOCUMENT_ID, DocumentsContract.Document.COLUMN_DISPLAY_NAME, DocumentsContract.Document.COLUMN_MIME_TYPE, DocumentsContract.Document.COLUMN_SIZE, DocumentsContract.Document.COLUMN_LAST_MODIFIED), null, null, null)?.use { cursor ->
                                while (cursor.moveToNext()) {
                                    ...
                                    if (mime == DocumentsContract.Document.MIME_TYPE_DIR) { results.put(JSONObject().apply { put("name", name); put("relPath", relPath); put("isDirectory", true); put("uri", DocumentsContract.buildDocumentUriUsingTree(uri, id).toString()) }); walkSaf(id, relPath, depth + 1) } 
                                    else { 
                                       var fSize = cursor.getLong(3); var fLast = cursor.getLong(4); val itemUri = DocumentsContract.buildDocumentUriUsingTree(uri, id)
                                       if (fSize <= 0) { val df = DocumentFile.fromSingleUri(this, itemUri); fSize = df?.length() ?: 0; fLast = df?.lastModified() ?: 0 }
                                       if (syncEverything || relPath.contains("nand/user/save") || allowedExts.contains(name.split(".").last().lowercase())) { results.put(JSONObject().apply { put("name", name); put("relPath", relPath); put("isDirectory", false); put("size", fSize); put("lastModified", fLast); put("uri", itemUri.toString()) }) } 
                                    }
                                }
                            }
                        }
                    ...
                    fun walkShizuku(currentPath: String, currentRelPath: String, depth: Int) {
                        ...
                        for (i in 0 until files.length()) {
                            ...
                            if (shouldIgnore(name, relPath)) continue; val fullPath = if (currentPath.endsWith("/")) "$currentPath$name" else "$currentPath/$name"
                            if (f.getBoolean("isDirectory")) { results.put(JSONObject().apply { put("name", name); put("relPath", relPath); put("isDirectory", true); put("uri", "shizuku://$fullPath") }); walkShizuku(fullPath, relPath, depth + 1) } 
                            else if (syncEverything || relPath.contains("nand/user/save") || allowedExts.contains(name.split(".").last().lowercase())) { results.put(JSONObject().apply { put("name", name); put("relPath", relPath); put("isDirectory", false); put("size", f.getLong("size")); put("lastModified", f.getLong("lastModified")); put("uri", "shizuku://$fullPath") }) }
                        }
                    }
                    ...
                    fun walkLocal(dir: File, currentRelPath: String) {
                        dir.listFiles()?.forEach { file ->
                            ...
                            else { if (syncEverything || relPath.contains("nand/user/save") || allowedExts.contains(file.name.split(".").last().lowercase())) { results.put(JSONObject().apply { put("name", file.name); put("relPath", relPath); put("isDirectory", false); put("size", file.length()); put("lastModified", file.lastModified()); put("uri", file.absolutePath) }) } }
                        }
                    }
                    walkLocal(File(path), "")
                }
    ...
    private fun getFileInfo(uriStr: String, result: MethodChannel.Result) {
        Thread {
            try {
                if (isShizukuPath(uriStr)) { val path = getCleanPath(uriStr); val service = getShizukuServiceSync(); val size = service.getFileSize(path); if (size != -1L) runOnUiThread { result.success(mapOf("size" to size, "lastModified" to service.getLastModified(path))) } else runOnUiThread { result.error("NOT_FOUND", "File not found", null) } } 
                else { val f = if (uriStr.startsWith("content://")) DocumentFile.fromSingleUri(this, Uri.parse(uriStr)) else DocumentFile.fromFile(File(uriStr)); if (f != null && f.exists()) runOnUiThread { result.success(mapOf("size" to f.length(), "lastModified" to f.lastModified())) } else runOnUiThread { result.error("NOT_FOUND", "File not found", null) } }
    ...
    private fun checkPathExists(path: String?): Boolean {
        ...
        else if (isShizukuPath(path)) (getShizukuServiceSync().getFileSize(getCleanPath(path)) != -1L) else File(path).exists()
    }

    private fun checkSafPermission(uriStr: String): Boolean {
        if (!uriStr.startsWith("content://")) return true; val targetUriStr = Uri.parse(uriStr).toString(); val permissions = contentResolver.persistedUriPermissions
        for (p in permissions) { if (p.uri.toString() == targetUriStr && p.isWritePermission) return true }; return false
    }
        ...
        Thread {
            try {
                ...
                if (uriStr.startsWith("content://")) {
                    val uri = Uri.parse(uriStr); val treeId = try { DocumentsContract.getTreeDocumentId(uri) } catch(e: Exception) { DocumentsContract.getDocumentId(uri) }
                    val childrenUri = DocumentsContract.buildChildDocumentsUriUsingTree(uri, treeId)
                    contentResolver.query(childrenUri, arrayOf(DocumentsContract.Document.COLUMN_DOCUMENT_ID, DocumentsContract.Document.COLUMN_DISPLAY_NAME, DocumentsContract.Document.COLUMN_MIME_TYPE), null, null, null)?.use { cursor ->
                        while (cursor.moveToNext()) {
                            val id = cursor.getString(0); val name = cursor.getString(1); val mime = cursor.getString(2); val isDir = mime == DocumentsContract.Document.MIME_TYPE_DIR; val itemUri = DocumentsContract.buildDocumentUriUsingTree(uri, id)
                            results.put(JSONObject().apply { put("name", name); put("uri", itemUri.toString()); put("isDirectory", isDir) })
                        }
                    }
                } else if (isShizukuPath(uriStr)) {
                    val cleanPath = getCleanPath(uriStr); val service = getShizukuServiceSync(); val json = service.listFileInfo(cleanPath); val files = JSONArray(json)
                    for (i in 0 until files.length()) { val f = files.getJSONObject(i); results.put(JSONObject().apply { put("name", f.getString("name")); put("uri", "shizuku://$cleanPath/${f.getString("name")}"); put("isDirectory", f.getBoolean("isDirectory")) }) }
                } else { val dir = File(uriStr); dir.listFiles()?.forEach { file -> results.put(JSONObject().apply { put("name", file.name); put("uri", file.absolutePath); put("isDirectory", file.isDirectory) }) } }
                runOnUiThread { result.success(results.toString()) }
    ...
    private fun checkShizukuStatus(): Map<String, Any> {
        ...
        try { if (Shizuku.pingBinder()) { status["running"] = true; status["authorized"] = Shizuku.checkSelfPermission() == PackageManager.PERMISSION_GRANTED; status["version"] = Shizuku.getLatestServiceVersion() } else { status["running"] = false; status["authorized"] = false } } 
        catch (e: Exception) { status["running"] = false; status["authorized"] = false; status["error"] = e.message ?: "Error" }; return status
    }
    ...
    private fun requestShizukuPermission(result: MethodChannel.Result) {
        ...
        val permissionListener = object : Shizuku.OnRequestPermissionResultListener { override fun onRequestPermissionResult(requestCode: Int, grantResult: Int) { Shizuku.removeRequestPermissionResultListener(this); runOnUiThread { result.success(grantResult == PackageManager.PERMISSION_GRANTED) } } }
        Shizuku.addRequestPermissionResultListener(permissionListener); Shizuku.requestPermission(1001)
    }

    private fun hasUsageStatsPermission(): Boolean {
        val appOps = getSystemService(Context.APP_OPS_SERVICE) as AppOpsManager; val mode = if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.Q) appOps.unsafeCheckOpNoThrow(AppOpsManager.OPSTR_GET_USAGE_STATS, android.os.Process.myUid(), packageName) else appOps.checkOpNoThrow(AppOpsManager.OPSTR_GET_USAGE_STATS, android.os.Process.myUid(), packageName)
    ...
    private fun getRecentlyClosedEmulator(emulatorPackages: List<String>): String? {
        ...
        val usageStatsManager = getSystemService(Context.USAGE_STATS_SERVICE) as UsageStatsManager; val time = System.currentTimeMillis(); val stats = usageStatsManager.queryUsageStats(UsageStatsManager.INTERVAL_DAILY, time - 300000, time)
        if (stats == null || stats.isEmpty()) return null; return stats.filter { emulatorPackages.contains(it.packageName) }.maxByOrNull { it.lastTimeUsed }?.packageName
    }

    private fun hasFilesWithExtensions(uriStr: String, extensions: List<String>, result: MethodChannel.Result) {
        Thread {
            try {
                val rootUri = Uri.parse(uriStr); val docId = try { DocumentsContract.getTreeDocumentId(rootUri) } catch(e: Exception) { DocumentsContract.getDocumentId(rootUri) }
                fun checkRecursive(currentDocId: String, depth: Int): Boolean {
                    if (depth > 3) return false; val childrenUri = DocumentsContract.buildChildDocumentsUriUsingTree(rootUri, currentDocId)
                    contentResolver.query(childrenUri, arrayOf(DocumentsContract.Document.COLUMN_DOCUMENT_ID, DocumentsContract.Document.COLUMN_DISPLAY_NAME, DocumentsContract.Document.COLUMN_MIME_TYPE), null, null, null)?.use { cursor ->
                        while (cursor.moveToNext()) { val id = cursor.getString(0); val name = cursor.getString(1).lowercase(); val mime = cursor.getString(2); if (mime == DocumentsContract.Document.MIME_TYPE_DIR) { if (checkRecursive(id, depth + 1)) return true } else { if (extensions.any { name.endsWith(".$it") }) return true } }
                    }; return false
