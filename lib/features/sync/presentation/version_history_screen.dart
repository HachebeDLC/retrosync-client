...
class _VersionHistoryScreenState extends ConsumerState<VersionHistoryScreen> {
  ...
  Future<void> _loadVersions() async {
    ...
    try {
      final serverVersions = await ref
          .read(syncRepositoryProvider)
          .getFileVersions(widget.remotePath);
  ...
  Future<List<Map<String, dynamic>>> _loadLocalBackups() async {
    ...
    try {
      final result = await _platform
          .invokeMethod('listLocalBackups', {'relPath': widget.relPath});
  ...
  Future<void> _restoreLocalBackup(String backupId, int displayNum) async {
    final confirmed = await showDialog<bool>(
      ...
      builder: (context) => AlertDialog(
        ...
        content: Text(
            'Restore local backup #$displayNum? This will overwrite the current local save.'),
        actions: [
          TextButton(
  ...
  Future<void> _restore(String versionId, int displayNum, int fileSize) async {
    final confirmed = await showDialog<bool>(
      ...
      builder: (context) => AlertDialog(
        ...
        content: Text(
            'Are you sure you want to restore Version $displayNum? Your current local save will be backed up before being overwritten.'),
        actions: [
          TextButton(
              onPressed: () => Navigator.pop(context, false),
              child: const Text('Cancel')),
  ...
  IconData _getDeviceIcon(String device) {
    ...
    if (d.contains('thor') || d.contains('ace') || d.contains('pocket')) {
                  ...
                  children: [
                    if (_localBackups.isNotEmpty) ...[
                      ...
                      const Padding(
                        ...
                        child: Text(
                            'Snapshots taken on this device just before a download overwrote your local save.',
                      ...
                      ..._localBackups.asMap().entries.map((entry) {
                        ...
                        final date = DateTime.fromMillisecondsSinceEpoch(
                            (b['updated_at'] as num).toInt());
                        final size =
                            ((b['size'] as num) / 1024).toStringAsFixed(1);
                        ...
                        return Card(
                          ...
                          child: ListTile(
                            ...
                            subtitle: Text('$size KB · local snapshot',
                                style: const TextStyle(fontSize: 12)),
                            trailing: OutlinedButton(
                              onPressed: () =>
                                  _restoreLocalBackup(backupId, displayNum),
                    ...
                    if (_versions.isNotEmpty) ...[
                      ...
                      const Padding(
                        padding: EdgeInsets.only(bottom: 8),
                        child: Text(
                            'Snapshots taken on the server before each upload overwrote the previous version.',
                            style: TextStyle(fontSize: 12, color: Colors.grey)),
                      ),
                      ..._versions.asMap().entries.map((entry) {
                        ...
                        final date = DateTime.fromMillisecondsSinceEpoch(
                            (v['updated_at'] as num).toInt());
                        final size =
                            ((v['size'] as num) / 1024).toStringAsFixed(1);
                        ...
                        return Card(
                          ...
                          child: ListTile(
                            ...
                            leading: Container(
                              ...
                              child: Icon(_getDeviceIcon(device),
                                  color: Colors.blue),
                            ),
                            title: Text(
                                DateFormat('MMM d, yyyy · HH:mm')
                            ...
                            trailing: OutlinedButton(
                              onPressed: () => _restore(
                                  versionId, displayNum, v['size'] as int),
                              child: const Text('RESTORE'),
                            ),
                          ),
                        );
    ...
    return Center(
      child: Column(
        ...
        children: [
          ...
          const Text('No previous versions available.',
              style: TextStyle(color: Colors.grey)),
          const SizedBox(height: 8),
          const Padding(
            padding: EdgeInsets.symmetric(horizontal: 48),
            child: Text(
                'Versions are created automatically when a file is overwritten or deleted.',
                style: TextStyle(fontSize: 12, color: Colors.grey),
